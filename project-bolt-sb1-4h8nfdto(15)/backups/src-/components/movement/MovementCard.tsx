...
export default function MovementCard({ exercise, onComplete, completedMovements }: MovementCardProps) {
  // Only check completed movements that match the current exercise ID
  const hasCompletedToday = completedMovements
    .filter(m => m.id === exercise.id)
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
      ...
      <div className="space-y-4">
        ...
        >
          {hasCompletedToday ? 'Completed Today' : 'Complete Movement'}
        </button>
      </div>
    </div>
  );
